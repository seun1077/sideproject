from __future__ import annotations

import urllib.parse
from pathlib import Path

from lxml import html

from ..http import fetch
from ..models import DealPostCandidate, ProductSeed
from ..repository import seed_key
from ..text_utils import clean, parse_price


def collect_algumon_latest(
    seeds: list[ProductSeed],
    raw_path: Path | None = None,
) -> list[DealPostCandidate]:
    url = "https://www.algumon.com/n/deal"
    status, body, error = fetch(url)
    if status != 200 or not body:
        return []

    if raw_path:
        raw_path.parent.mkdir(parents=True, exist_ok=True)
        raw_path.write_text(body, encoding="utf-8")

    keywords: list[tuple[str, ProductSeed]] = []
    for seed in seeds:
        for part in [seed.brand, seed.product, seed.query, seed.category]:
            for token in part.split():
                token = token.strip()
                if len(token) >= 2:
                    keywords.append((token, seed))
    keywords = sorted(keywords, key=lambda pair: len(pair[0]), reverse=True)

    doc = html.fromstring(body)
    posts: list[DealPostCandidate] = []
    seen = set()
    for anchor in doc.xpath("//a[@href]"):
        title = clean(" ".join(anchor.xpath(".//text()")))
        href = anchor.get("href")
        if not title or not href:
            continue
        matched = [(keyword, seed) for keyword, seed in keywords if keyword in title]
        if not matched:
            continue
        if href.startswith("/"):
            href = urllib.parse.urljoin("https://www.algumon.com", href)
        key = (title, href)
        if key in seen:
            continue
        seen.add(key)
        best_seed = matched[0][1]
        posts.append(
            DealPostCandidate(
                source_code="algumon",
                product_key=seed_key(best_seed),
                title=title[:240],
                url=href,
                extracted_price_krw=parse_price(title),
                matched_keywords=",".join(keyword for keyword, _ in matched[:8]),
                match_score=min(100, len(matched) * 10),
                raw_payload={"http_status": status, "error": error},
            )
        )
    return posts

